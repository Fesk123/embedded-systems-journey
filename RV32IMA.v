...
module RV32IMA_CPU (
    ...
    always @(*) begin
        ...
        case (opcode)
            ...
            OPCODE_OP_IMM, OPCODE_LOAD, OPCODE_JALR: immediate = {{20{instruction[31]}}, instruction[31:20]};
    ...
    always @(*) begin
        ...
        case (alu_control)
            ...
            ALU_SRA: alu_result = $signed(alu_input_a) >>> alu_input_b[4:0];
    ...
    // Implements the RV32M multiply and divide instructions.

    reg [31:0] m_result;
    reg signed [63:0] signed_multiply;

    reg signed [63:0] signed_multiply_hsu;
    reg [63:0] unsigned_multiply;

    always @(*) begin

        signed_multiply = $signed({{32{rs1_data[31]}}, rs1_data}) * $signed({{32{rs2_data[31]}}, rs2_data});
        signed_multiply_hsu = $signed({{32{rs1_data[31]}}, rs1_data}) * $signed({32'd0, rs2_data});
        unsigned_multiply = {32'd0, rs1_data} * {32'd0, rs2_data};
        ...
        case (funct3)
            ...
            // MUL
            3'b000: m_result = signed_multiply[31:0];

            // MULH
            3'b001: m_result = signed_multiply[63:32];

            // MULHSU
            3'b010: m_result = signed_multiply_hsu[63:32];

            // MULHU
            3'b011: m_result = unsigned_multiply[63:32];

            // DIV
            3'b100: begin

                if (rs2_data == 32'd0)
                    m_result = 32'hFFFFFFFF;
                else if ((rs1_data == 32'h80000000) && (rs2_data == 32'hFFFFFFFF))
                    m_result = 32'h80000000;
                else
    ...
    // The atomic instructions read a memory word, calculate a new value,
    // and write the new value back as one atomic operation in this CPU.

    wire [31:0] atomic_address;
    wire [31:0] atomic_memory_data;
    reg [31:0] atomic_result;
    reg [31:0] atomic_old_value;
    reg atomic_sc_success;

    assign atomic_address = rs1_data;
    assign atomic_memory_data = data_memory[atomic_address[31:2]];
    ...
    always @(*) begin
        ...
        if (is_sc) begin
            ...
            atomic_result = atomic_sc_success ? 32'd0 : 32'd1;
        end
    ...
    always @(*) begin
        ...
        if (is_m_instruction) begin
            ...
            case (result_select)
                ...
                // PC + 4
                2'b10: write_back_data = pc + 32'd4;
        ...
        if (rst) begin
            ...
            if (is_sc) begin
                ...
                if (atomic_sc_success)
                    data_memory[atomic_address[31:2]] <= rs2_data;
            ...
            if (is_atomic_instruction && atomic_write) begin
                data_memory[atomic_address[31:2]] <= atomic_result;
                reservation_valid <= 1'b0;
            end
